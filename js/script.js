document.addEventListener('DOMContentLoaded', () => {
    ...
    function fetchAllWeatherData() {
        ...
        const shortTermPromise = fetchShortTermWeatherData();
        const midTermPromise = fetchMidTermWeatherData();

        Promise.allSettled([shortTermPromise, midTermPromise])
            .then(([shortTermResult, midTermResult]) => {
                let hasError = false;
                if (shortTermResult.status === 'fulfilled' && shortTermResult.value) {
                    originalForecastItems = shortTermResult.value;
                } else {
                    console.error("단기예보 API 호출 실패:", shortTermResult.reason);
                    hasError = true;
                }
                if (midTermResult.status === 'fulfilled' && midTermResult.value) {
                    originalMidTermForecast = midTermResult.value;
                } else {
                    console.error("중기예보 API 호출 실패:", midTermResult.reason);
                }
                if (hasError) throw new Error("필수 API(단기예보) 호출에 실패했습니다.");
                updateUI(originalForecastItems, originalMidTermForecast);
    ...
    function fetchShortTermWeatherData() {
        ...
        if (hours < 2 || (hours === 2 && minutes < 10)) { const yesterday = new Date(now.getTime() - 24 * 60 * 60 * 1000); base_date = `${yesterday.getFullYear()}${String(yesterday.getMonth() + 1).padStart(2, '0')}${String(yesterday.getDate()).padStart(2, '0')}`; base_time = "2000"; }
        else if (hours < 5 || (hours === 5 && minutes < 10)) base_time = "2300";
        ...
        const fullUrl = `${VILAGE_FCST_URL}?${new URLSearchParams({ authKey: API_KEY, pageNo: '1', numOfRows: '1000', dataType: 'JSON', base_date, base_time, nx: '98', ny: '125' })}`;
    ...
    function fetchMidTermWeatherData() {
        ...
        if (kstHours < 7) {
            ...
            announcementTime = "1800";
        } else if (kstHours < 19) {
            announcementTime = "0600";
        } else {
            announcementTime = "1800";
        }
        ...
        return fetch(`${proxyUrl}${encodeURIComponent(fullUrl)}`).then(res => {
    ...
    function calculateRisk(hourlyData, sortedKeys) {
        ...
        if (currentScore >= 40) { level = "심각"; className = "level-severe"; recommendation = "눈깨비가 경고해요! 매우 위험한 폭설이 예상돼요. 지금은 꼭 안전한 실내에 머무르는 것이 가장 중요해요."; }
        else if (currentScore >= 30) { level = "경계"; className = "level-warning"; recommendation = "눈깨비가 걱정돼요! 여러 위험이 함께 예상되니, 차량 운행은 자제하고 외출 시 각별히 조심해 주세요."; }
        else if (currentScore >= 20) { level = "주의"; className = "level-caution"; recommendation = "눈깨비가 알려드려요! 눈이 제법 많이 올 수 있어요. 운전하신다면 미끄럼 사고에 꼭 주의해 주세요."; }
        else if (currentScore > 0) { recommendation = "눈깨비가 알려드려요! 가벼운 눈 소식이 있어요. 최신 정보를 계속 확인할게요."; }
        else { level = "안전"; className = ""; }
    ...
    function updateCardColors(sky, pty) {
        ...
        let newClass = ''; pty = parseInt(pty); sky = parseInt(sky);
    ...
    function updateUI(forecastItems, midTermData, isTestMode = false, scenarioData = null, riskOverride = null) {
        ...
        let currentData;
        if (isTestMode && scenarioData) { currentData = scenarioData; } else { const currentDataKey = sortedKeys.find(key => key >= `${currentDateString}_${now.getHours().toString().padStart(2,'0')}00`) || sortedKeys[0]; currentData = hourlyData[currentDataKey] || {}; }
        ...
        let skyText; const pty = parseInt(currentData.PTY), sky = parseInt(currentData.SKY);
        if (pty > 0) { if (pty === 1 || pty === 4) skyText = '비'; else if (pty === 2) skyText = '진눈깨비'; else if (pty === 3) skyText = '눈'; else skyText = '강수'; }
        else { if (sky === 1) skyText = '맑음'; else if (sky === 3) skyText = '구름많음'; else if (sky === 4) skyText = '흐림'; else skyText = '정보없음'; }
        ...
        briefingCard.style.display = 'block'; const allLevelClasses = ['level-interest', 'level-caution', 'level-warning', 'level-severe']; briefingCard.classList.remove(...allLevelClasses);
        ...
        updateActionGuide(riskAnalysis.level);
        if(midTermData && !isTestMode) updateWeeklyForecast(midTermData);
        updateCardColors(currentData.SKY, currentData.PTY);
        hourlyContainer.innerHTML = '';
        let startIndex;
        if (isTestMode && scenarioData) { const currentTestKey = `${scenarioData.fcstDate}_${scenarioData.fcstTime}`; const currentTestIndex = sortedKeys.findIndex(key => key === currentTestKey); startIndex = currentTestIndex !== -1 ? Math.max(0, currentTestIndex - 2) : 0; }
        else { const currentDataKey = `${currentDateString}_${now.getHours().toString().padStart(2, '0')}00`; startIndex = Math.max(0, sortedKeys.findIndex(key => key >= currentDataKey) - 2); }
        const endIndex = Math.min(sortedKeys.length, startIndex + 50), displayKeys = sortedKeys.slice(startIndex, endIndex); let lastDate = displayKeys.length > 0 ? hourlyData[displayKeys[0]].fcstDate : '';
    ...
    function updateSnowAlerts(snowAlerts, hourlyData) {
        const snowAlertCard = document.querySelector('.snow-alert-card'); const snowAlertsList = snowAlertCard.querySelector('ul'); snowAlertsList.innerHTML = '';
        if (snowAlerts.length > 0) {
            snowAlertCard.style.display = 'block'; const processedTimes = new Set();
            snowAlerts.forEach(alert => {
                const key = `${alert.fcstDate}_${alert.fcstTime}`; if (processedTimes.has(key)) return;
                const hourData = hourlyData[key] || {}; const li = document.createElement('li'); let warning = "차량 운행 시 미끄럼에 주의하세요.";
                if (parseFloat(alert.fcstValue) >= 3.0) warning = "대설주의보 수준의 눈입니다. 가급적 외출을 자제하세요.";
                if (parseFloat(hourData.WSD) >= 9.0) warning += " 강풍을 동반하여 눈보라가 칠 수 있습니다.";
                li.innerHTML = `<div class="alert-time">${parseInt(alert.fcstTime.substring(0,2))}시 예보</div><div class="alert-details"><span>적설: ${alert.fcstValue}cm</span> | <span>기온: ${hourData.TMP}°C</span> | <span>풍속: ${hourData.WSD}m/s</span></div><div class="alert-warning">${warning}</div>`;
                snowAlertsList.appendChild(li); processedTimes.add(key);
            });
        } else { snowAlertCard.style.display = 'none'; }
    }

    function updateActionGuide(riskLevel) {
        const contentEl = document.getElementById('action-guide-content');
        const guides = {
            '안전': `<h4>사전 준비</h4><ul><li>TV, 라디오, 스마트폰 등으로 기상정보를 수시로 확인합니다.</li><li>내 집 앞, 내 점포 앞 도로의 눈을 치울 준비를 합니다.</li><li>비상용품(응급약품, 손전등, 식수, 비상식량 등)을 미리 준비합니다.</li></ul>`,
            '관심': `<h4>대설 예보 시</h4><ul><li>차량용 월동용품(스노체인, 삽, 염화칼슘 등)을 준비하고, 운행 시에는 저속 운행 및 안전거리를 확보합니다.</li><li>어린이와 노약자는 외출을 자제합니다.</li><li>산간 고립 우려 지역은 식량, 연료 등 비상용품을 준비합니다.</li></ul>`,
            '주의': `<h4>대설주의보 발령 중</h4><ul><li>가급적 대중교통을 이용하고, 운전 시에는 고속도로 진입을 자제하고 국도를 이용합니다.</li><li>보온을 위해 외투, 장갑, 모자 등을 착용하고, 미끄럼 방지 신발을 신습니다.</li><li>수도계량기, 보일러 등은 동파되지 않도록 보온을 강화합니다.</li><li>붕괴가 우려되는 비닐하우스 등 농업 시설물은 받침대를 보강하는 등 사전 조치합니다.</li></ul>`,
            '경계': `<h4>대설경보 발령 중</h4><ul><li>외출을 최대한 자제하고, 차량 운행을 멈추고 안전한 곳으로 이동합니다.</li><li>차량이 고립된 경우, 119에 신고하고 차 안에서 구조를 기다립니다. (주기적으로 창문 환기)</li><li>주변의 독거노인 등 취약계층의 안부를 확인합니다.</li><li>노후 주택은 쌓인 눈의 무게로 무너질 위험이 있으니 안전점검을 실시합니다.</li></ul>`,
            '심각': `<h4>극심한 재난 상황</h4><ul><li>외출을 절대적으로 삼가고, 안전한 실내에 머무르며 재난 방송에 집중합니다.</li><li>TV, 라디오가 없을 경우, 핸드폰 등을 통해 재난문자 수신에 집중합니다.</li><li>차량이 고립된 경우, 가능한 모든 수단으로 구조 연락을 취하고, 색깔 있는 옷을 눈 위에 펼쳐 구조요원이 쉽게 찾을 수 있도록 합니다.</li></ul>`
        };
        contentEl.innerHTML = guides[riskLevel] || `<p>현재 위험도에 맞는 행동요령 정보가 없습니다.</p>`;
    }

    function updateWeeklyForecast(midTermData) {
        const container = document.getElementById('weekly-container');
        if (!midTermData) { container.innerHTML = '<p>주간 예보 정보를 불러올 수 없습니다.</p>'; return; }
        container.innerHTML = ''; const weekdays = ['일', '월', '화', '수', '목', '금', '토'];
        for (let i = 3; i <= 7; i++) {
            const wfKey = `wf${i}`, taMinKey = `taMin${i}`, taMaxKey = `taMax${i}`;
            if (midTermData[wfKey] === undefined) continue;
            const date = new Date(); date.setDate(date.getDate() + i); const dayName = weekdays[date.getDay()];
            const itemDiv = document.createElement('div'); itemDiv.className = 'weekly-item';
            let skyText = midTermData[wfKey], iconSvg = '';
            if (skyText.includes('맑음')) iconSvg = getWeatherIconSVG(1, 0);
            else if (skyText.includes('구름많음')) iconSvg = getWeatherIconSVG(3, 0);
            else if (skyText.includes('흐림')) iconSvg = getWeatherIconSVG(4, 0);
            else if (skyText.includes('비')) iconSvg = getWeatherIconSVG(4, 1);
            else if (skyText.includes('눈')) iconSvg = getWeatherIconSVG(4, 3);
            else iconSvg = getWeatherIconSVG(3, 0);
            itemDiv.innerHTML = `<div class="day">${date.getDate()}일 (${dayName})</div><div class="icon">${iconSvg}</div><div class="temps"><span class="max">${midTermData[taMaxKey]}°</span> / <span class="min">${midTermData[taMinKey]}°</span></div>`;
    ...
    function applyTestScenario(scenarioKey) {
        ...
        testItems.forEach(item => { const itemDate = new Date(item.fcstDate.replace(/(\d{4})(\d{2})(\d{2})/, '$1-$2-$3') + 'T' + item.fcstTime.replace(/(\d{2})(\d{2})/, '$1:$2:00')); const hoursDiff = Math.round((itemDate.getTime() - now.getTime()) / 3600000); const matchingScenario = scenario.find(s => s.offset === hoursDiff); if (matchingScenario) { const s = matchingScenario; if (item.category === 'TMP') item.fcstValue = s.tmp; if (item.category === 'PTY') item.fcstValue = s.pty; if (item.category === 'SKY') item.fcstValue = s.sky; if (item.category === 'SNO') item.fcstValue = s.sno; if (item.category === 'WSD') item.fcstValue = s.wsd; if (s.offset === 0) scenarioCurrentData = { fcstDate: item.fcstDate, fcstTime: item.fcstTime, TMP: s.tmp, PTY: s.pty, SKY: s.sky, SNO: s.sno, WSD: s.wsd, REH: '50' }; } });
        scenario.forEach(s => { if (parseFloat(s.sno) > 0) { const scenarioDate = new Date(now.getTime() + s.offset * 3600000); const fcstDate = `${scenarioDate.getFullYear()}${String(scenarioDate.getMonth() + 1).padStart(2, '0')}${String(scenarioDate.getDate()).padStart(2, '0')}`; const fcstTime = `${String(scenarioDate.getHours()).padStart(2, '0')}00`; const existingSnoItem = testItems.find(item => item.category === 'SNO' && item.fcstDate === fcstDate && item.fcstTime === fcstTime); if (existingSnoItem) existingSnoItem.fcstValue = s.sno; else testItems.push({ base_date: fcstDate, base_time: fcstTime, category: 'SNO', fcstDate: fcstDate, fcstTime: fcstTime, fcstValue: s.sno, nx: 98, ny: 125 }); } });
        const scenarioTemps = scenario.map(s => parseFloat(s.tmp)); const tmn = Math.min(...scenarioTemps), tmx = Math.max(...scenarioTemps);
        const tmnItem = testItems.find(i => i.category === 'TMN'); if(tmnItem) tmnItem.fcstValue = String(tmn);
        const tmxItem = testItems.find(i => i.category === 'TMX'); if(tmxItem) tmxItem.fcstValue = String(tmx);
        const riskDetails = testScenarioDetails[scenarioKey];
        updateUI(testItems, originalMidTermForecast, true, scenarioCurrentData, riskDetails);
    }
    ...
    document.getElementById('risk-details-toggle').addEventListener('click', (e) => {
        const toggle = e.currentTarget, breakdown = document.getElementById('risk-breakdown');
        toggle.classList.toggle('expanded');
        breakdown.style.display = toggle.classList.contains('expanded') ? 'block' : 'none';
        toggle.childNodes[0].nodeValue = toggle.classList.contains('expanded') ? '닫기' : '자세히 보기';
    });
    
    let keySequence = '';
    document.addEventListener('keydown', (e) => {
        keySequence += e.key; keySequence = keySequence.slice(-3);
        if (testScenarios[keySequence]) { applyTestScenario(keySequence); keySequence = ''; loaderElement.classList.add('hidden'); dashboardElement.classList.remove('hidden'); }
        else if (keySequence === '000') { isTestModeActive = false; updateUI(originalForecastItems, originalMidTermForecast); keySequence = ''; dashboardElement.classList.remove('hidden'); }
    });
    
    const savedTheme = localStorage.getItem('theme');
    const prefersDark = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;
    if (savedTheme === 'dark' || (!savedTheme && prefersDark)) { document.body.classList.add('dark-mode'); darkModeSwitch.checked = true; }
    darkModeSwitch.addEventListener('change', () => { document.body.classList.toggle('dark-mode'); localStorage.setItem('theme', document.body.classList.contains('dark-mode') ? 'dark' : 'light'); });
    setInterval(() => { const now = new Date(); if (now.getMinutes() === 10 && now.getHours() % 3 === 0) fetchAllWeatherData(); else if (!dashboardElement.classList.contains('hidden') && !isTestModeActive) updateUI(originalForecastItems, originalMidTermForecast); }, 60000);
